...
import {plantDiscoveryName} from '../visualizer/sphere-simulation-panel.js';
...
test('every biological species references an existing vector symbol and stable unique ID',()=>{
 ...
 assert.equal(new Set([...catalog.crops,...catalog.animals].map(s=>s.id)).size,33);
});
...
test('plant discovery journal distinguishes food crops from technical crops',()=>{
 assert.equal(plantDiscoveryName({type:'plant_discovered',details:{species:'wheat'}},catalog),'Найдено съедобное растение');
 assert.equal(plantDiscoveryName({type:'plant_discovered',details:{species:'cotton'}},catalog),'Найдено техническое растение');
 assert.equal(plantDiscoveryName({type:'plant_discovered',details:{species:'unknown'}},catalog),'Найдено растение');
});
